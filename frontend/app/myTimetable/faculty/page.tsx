...
// buildGrid now takes slots from DB instead of only from entries
...
// Simple light theme loading animation
...
export default function MyTimetablePage() {
    ...
    async function fetchTimetableAndSlots() {
      ...
      try {
        ...
        const res2 = await fetch(
          `${API_BASE}/timetables/faculty/${data.faculty.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  if (error)
    return (
      <div className="p-6 text-red-500 bg-white text-black">{error}</div>
    );
  if (!entries.length)
    return (
      <div className="p-6 bg-white text-black">No timetable</div>
    );
  if (!slots.length)
    return (
      <div className="p-6 bg-white text-black">No slots defined</div>
    );
