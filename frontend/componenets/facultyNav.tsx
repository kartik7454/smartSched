...
export default function FacultyNav({ logout }: { logout: () => void }) {
  const [isOpen, setIsOpen] = useState(false);
  const router = useRouter();

  // Responsive nav design, matching style of hodNav, with icons and nice hover/focus styles
  return (
    <nav className="sticky top-2 z-40 shadow-xl bg-gradient-to-r from-white via-gray-50 to-blue-50 rounded-2xl px-8 py-4 w-full max-w-3xl mx-auto mt-4 transition-shadow mb-4 border border-blue-100">
      <div className="flex justify-between items-center sm:hidden mb-2">
        <button
          ...
          className="p-2 rounded focus:outline-none focus:ring bg-blue-50"
        ...
        <span className="text-lg font-black tracking-wide text-blue-900 ml-2 select-none">Faculty Panel</span>
      </div>
